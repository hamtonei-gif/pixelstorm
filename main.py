...
def start_game():
    ...
    root.after(0, root.withdraw)  # Hide tkinter window safely from thread
    ...
    # CAP_DSHOW is more reliable in frozen .exe on Windows
    cap = cv2.VideoCapture(camera_index, cv2.CAP_DSHOW)
 
    if not cap.isOpened():
        print(f"Camera {camera_index} with DSHOW failed, trying without...")
...
def open_employee_form():
    ...
    def submit():
        ...
        if employee_name and employee_id:
            form.destroy()
            # Run game in separate thread so tkinter doesn't block OpenCV
